...
int main()
{
	res = test_add(8,7);
	 ...
	 // Test of os_Malloc and os_free
	   void* memory = os_alloc(1024); // Allocate memory
	    if (memory != NULL) {
	        printf("La mémoire est bien allouée à l'adresse: %p\n", memory);
	        // Pour liberer le pointeur
	        os_free(memory);
	        printf("La mémoire est bien liberée.\n");
	    } else {
...
int main()
{
    ...
    printf("La %d tache est bien crée \n", task_new(code1, 512));
    printf("La %d tache est bien crée\n", task_new(code2, 512));
    printf("La %d tache est bien crée\n", task_new(code3, 512));


    printf("Début execution des taches\n");
